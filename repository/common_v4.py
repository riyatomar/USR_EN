...
from repository.verb import Verb
from repository.concept import Concept
from repository.inputForMorphGen import format_morph_input
...
def populate_spkview_dict(spkview_info,discourse_data,index_data):
    ...
    for i, info in enumerate(spkview_info):
        ...
        if clean_spk_info in repository.constant.spkview_list_b and 'def' in clean_spk_info:
            temp = (b, 'the')
...
def convert_to_devanagari(text):
    ...
    devanagari_text = text #transliterate(text, sanscript.ITRANS, sanscript.DEVANAGARI)
...
def preprocess_postposition_new(concept_type, np_data, words_info, verb_data, index_data):
    ...
    if data_case in ('k1', 'pk1'):
    ...
    elif data_case in ['k3', 'rask1']:
        ppost = 'with'
    elif data_case in ('k2p', 'k4'):
        ppost = 'to' 
    elif data_case in ['k5', 'k5prk']:
        ppost = 'from'
    elif data_case == 'k7':
        ppost = 'on'
    elif data_case == 'k7t':
        ...
        ppost = 'according to'
    elif data_case in ['rask2', 'rask3', 'rask4', 'rask5', 'rask7']:
        ppost = 'along with'
    elif data_case in ['rasnegk1', 'rasnegk2', 'rasnegk3']:
        ppost = 'without'
    elif data_case in ['rblak', 'rpk']:
        ppost = 'after'
    elif data_case in ['rblpk']:
        ppost = 'before'
    elif data_case == 'rt':
        ppost = 'for'
    elif data_case == 'r6':
...
def all_processed_cat(index_data,gnp_data,seman_data,depend_data,spkview_data,sentence_type,words_info,k1_not_need,has_changes):
    ...
    print('processed category----------->',foreign_words_data,indeclinables_data, pronouns_data, nouns_data,verbal_adjectives, adjectives_data, verbs_data, adverbs_data, others_data, nominal_forms_data)
...
def process_main_verb(concept: Concept, seman_data, gnp_data, dependency_data, sentence_type, processed_nouns, processed_pronouns,index_data, reprocessing,k1_not_need):
    ...
    if sentence_type[1:] not in ("fragment","title","heading","term"):
        ...
        if verb.term == 'be' and verb.tam in ('pres', 'past'):  # process TAM
            # alt_tam = {'pres': 'pres', 'past': 'past'}
            # alt_root = {'pres': 'be', 'past': 'be'}
...
def identify_tam_terms(term):
    """
    >>> def identify_tam_terms:("kara_1-wA_hE_1")
    wA_hE_1
    >>> def identify_tam_terms:("kara_1-0_rahA_hE_1")
    0_rahA_hE_1
    """
    tam = term.split("-")[1]
    return tam
...
def find_tags_from_dix(word):
    ...
    """
    dix_command = "echo {} | apertium-destxt | lt-proc -ac apertium_eng/eng.automorf.bin | apertium-retxt".format(word)
...
#             if data[5] == 's':
#                 num = 'sg'
#             morph_data = f'^{data[1]}<{data[2]}><{num}>$'
...
#         # elif data[2] == 'v' and data[8] in ('main','auxiliary'):
#         #     morph_data = f'^{data[1]}<vblex><{data[6]}><p3><sg>$'
#         elif data[2] == 'v' and data[8] == 'main':
#             morph_data = f'^{data[1]}<vblex><pprs>$'
...
def run_morph_generator(data):
    ...
    print("# morph_generator_result================>",result.stdout)
...
def generate_morph(processed_words, tam_term, depend_data, sentence_type):
    """Run Morph generator"""
    morph_input = generate_input_for_morph_generator(processed_words, tam_term, depend_data, sentence_type)
...
#         print('ooooooooooooooo', output_data, 'mmmmmmmmmmmm', morph_input)
#     combine_data = []
#     for i in range(len(output_data)):
#         morph_input_list = list(morph_input[i])
#         # if '#' in morph_input_list[-1]:
#         #     morph_input_list[1] = output_data[i+1]
#         # else:
#         morph_input_list[1] = output_data[i]
#         combine_data.append(tuple(morph_input_list))
#         print(morph_input_list, 'uuuuuuuuuuuuuuuuuuuuuuuuu')
#     return combine_data
...
def analyse_output_data(output_data, morph_input):
    ...
    for i in range(len(morph_input)):
        morph_input_list = list(morph_input[i])
        
        if '#' in morph_input_list[-1]:
            phrase_length = len(morph_input_list[-1].split())  # Checking multi-word phrase
            morph_input_list[1] = ' '.join(output_data[output_index:output_index + phrase_length])
            output_index += phrase_length
        else:
            morph_input_list[1] = output_data[output_index]
            output_index += 1
...
def add_postposition(transformed_fulldata,index_data,depend_data, processed_postpositions):
    ...
    for data in transformed_fulldata:
        ...
        if index in processed_postpositions:
            ...
            if ppost != None and (temp[2] == 'n'or temp[2] == 'np' or temp[2] == 'vn' or temp[2] == 'other'):
                temp[1] = temp[1] + ' ' + ppost
            data = tuple(temp)
        ...
        for data_values in discourse_data:
            if data_values!='' and 'coref' not in data_values:
                ...
                if 'BI_1' in spkview_data:
                    ...
                    break
                # elif 'def' in spkview_data:
                #     POST_PROCESS_OUTPUT = 'the' + POST_PROCESS_OUTPUT
                #     break
